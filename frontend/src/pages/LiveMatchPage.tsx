...
export const LiveMatchPage: React.FC = () => {
  ...
  const handleRegisterAndAnalyze = async () => {
    ...
    try {
      ...
      const body = {
        ...
        tag,
        recent_points: recordedPoints.slice(-15).map(pt => ({
          isPointWon: pt.isPointWon,
          macroPattern: pt.macroPattern,
          rallyCount: pt.rallyCount,
          isOnServe: pt.isOnServe,
          serveNumber: pt.serveNumber === "ACE" ? 1 : pt.serveNumber
        })),
      };
      ...
      const rec: RecordedPoint = {
        ...
        strategicPriority: data.tactical_v3?.strategic_priority,
        matchPlan: data.tactical_v3?.match_plan,
        dominantZone: data.tactical_v3?.dominant_zone,
        vulnerabilityZone: data.tactical_v3?.vulnerability_zone,
        recommendedIntent: data.tactical_v3?.recommended_intent,
        setScoreMe: setsMe,
        setScoreOpp: setsOpp,
        gameScoreMe: gamesMe,
  ...
  const fallbackImmediateTacticalCall = buildImmediateTacticalCall({
    prediction,
    taggedPrediction,
  ...
  const immediateTacticalCall =
    prediction?.tactical_call?.trim() || fallbackImmediateTacticalCall;
  ...
  if (isSettingUp) {
    return (
      <LiveMatchSetup
        ...
        newPlayerHandedness={newPlayerHandedness}
        newPlayerPlayStyle={newPlayerPlayStyle}
        newPlayerNotes={newPlayerNotes}
        ...
        onFirstServerChange={setFirstServer}
        onRoundChange={setRound}
        onSaveNewPlayer={handleSaveNewPlayer}
  ...
  return (
    <div className="flex flex-col gap-4 h-full">
      <div className="bg-slate-950/70 border border-slate-800 rounded-[24px] p-6 lg:p-8 flex flex-col gap-6 relative shadow-[0_2px_40px_-15px_rgba(0,0,0,0.5)]">
        ...
        <LiveMatchHero
          ...
          setsMe={setsMe}
          gamesMe={gamesMe}
          pointScoreMe={pointScoreMe}
          setsOpp={setsOpp}
          gamesOpp={gamesOpp}
          pointScoreOpp={pointScoreOpp}
          onServe={onServe}
      ...
      <div className="grid grid-cols-1 xl:grid-cols-[minmax(0,1.35fr)_minmax(0,1fr)] gap-4">
        ...
        <div className="flex flex-col gap-4">
          <LiveStatsPanel
            ...
            firstPct={firstPct}
            secondPct={secondPct}
            momentumLast5={momentumLast5}
            onSvcPctChange={setSvcPct}
            onRtnPctChange={setRtnPct}
            onFirstPctChange={setFirstPct}
            onSecondPctChange={setSecondPct}
